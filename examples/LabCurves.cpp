...
void LabCurves::Render(float inAspectRatio) {
	mat4 model;
	mat4 view = lookAt(vec3(0, 0, 5), vec3(0, 0, 0), vec3(0, 1, 0));
	mat4 projection = orthographic(0, inAspectRatio * 22.0f, 0, 22, 0.001f, 10);
...
ScalarFrame LabCurves::MakeFrame(float time, float value) {
	return MakeFrame(time, 0.0f, value, 0.0f);
}

VectorFrame LabCurves::MakeFrame(float time, const vec3& value) {
	return MakeFrame(time, vec3(), value, vec3());
}

QuaternionFrame LabCurves::MakeFrame(float time, const quat& value) {
	return MakeFrame(time, quat(0, 0, 0, 0), value, quat(0, 0, 0, 0));
}

ScalarFrame LabCurves::MakeFrame(float time, float in, float value, float out) {
	ScalarFrame result;
	result.mTime = time;
	result.mIn[0] = in;
...
VectorFrame LabCurves::MakeFrame(float time, const vec3& in, const vec3& value, const vec3& out) {
	VectorFrame result;
	result.mTime = time;
	result.mIn[0] = in.x;
	result.mIn[1] = in.y;
...
QuaternionFrame LabCurves::MakeFrame(float time, const quat& in, const quat& out, const quat& value) {
...
ScalarTrack LabCurves::MakeScalarTrack(Interpolation interp, int numFrames, ...) {
	ScalarTrack result;
...
VectorTrack LabCurves::MakeVectorTrack(Interpolation interp, int numFrames, ...) {
	VectorTrack result;
	result.SetInterpolation(interp);
	result.Resize(numFrames);
...
QuaternionTrack LabCurves::MakeQuaternionTrack(Interpolation interp, int numFrames, ...) {
