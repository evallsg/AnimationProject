...
Pose& Pose::operator=(const Pose& p) {
	//make sure the pose isn't assigned to itself
	if (&p == this) {
		return *this;
	}
	//make sure that the pose has the correct number of joints and parents
...
Transform Pose::GetGlobalTransform(unsigned int i) {
	...
	//Combines all the transforms up the parent chain until it reaches the root bone
...
void Pose::GetMatrixPalette(std::vector<mat4>& out) { //not omptimized
	unsigned int size = Size();
	if (out.size() != size) {
		out.resize(size);
	}
	//For every transform, find the global transform, convert it into a matrix, and store the result in the vector of matrices
...
bool Pose::operator==(const Pose& other) {
	...
	//make sure that all of the joint transforms and parent indices are identical in both poses
